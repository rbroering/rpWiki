...
class PageBase {
	...
	protected function __insertCheckbox($fetch) {
		foreach ($fetch as $id => $checkBox) {
			...
			?>
			<div class="checkbox" >
				<input type="checkbox" name="<?php echo $checkBox['name']; ?>" id="<?php echo $id; ?>" class="check-hidden<?php if (!empty($checkBox['class'])) echo ' ' . $checkBox['class']; ?>" <?php if ($checkBox['checked']) { ?>checked="checked" <?php } ?>/>
